...
from PyQt5.QtGui import QPainter, QColor, QRadialGradient, QColor, QPainter, QPainterPath, QFont
...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        keyboard.add_hotkey('Space', self.love_button.click)
        keyboard.add_hotkey('Ctrl', self.star.click)
        keyboard.add_hotkey('Ctrl+Space', self.music_button.click)
        keyboard.add_hotkey('Ctrl+Right Arrow', self.next_music_button.click)
        keyboard.add_hotkey('Ctrl+Left Arrow', self.last_music_button.click)
    ...
    def resizeEvent(self, event):
        ...
        def adjust_widget2(widget, x_ratio, y_ratio, width_ratio, height_ratio):
            widget.setGeometry(int(width * x_ratio), int(height * y_ratio),
                                            int(length * width_ratio), int(length * height_ratio))

        # 定义辅助函数，用于调整按钮的图标大小
        def set_icon_size(button, width_ratio, height_ratio=None):
            icon_height = int(length * height_ratio)
            icon_width = int(length * width_ratio)
            button.setIconSize(QtCore.QSize(icon_width, icon_height))

        def set_font_size(widget, font_name, length_ratio, reference_length):
            font_size = int(reference_length * length_ratio)
            font = QFont(font_name, font_size)
            widget.setFont(font)

        length = math.sqrt(width * height)
        ...
        set_icon_size(self.ui.love_button, 0.09, 0.09)
        set_icon_size(self.ui.last_button, 0.085, 0.085)
        set_icon_size(self.ui.next_button, 0.085, 0.085)
        set_icon_size(self.ui.search_confirm_button, 0.04, 0.06)
        set_icon_size(self.ui.music_button, 0.045, 0.045)
        set_icon_size(self.ui.last_music_button, 0.045, 0.045)
        set_icon_size(self.ui.next_music_button, 0.045, 0.045)

        # 使用辅助函数调整字体大小
        set_font_size(self.ui.label, "Microsoft YaHei", 0.007, length)
        set_font_size(self.ui.date, "STLiti", 0.02, length)
        set_font_size(self.ui.sen_num, "STLiti", 0.006, length)
        set_font_size(self.ui.hover_label, "Microsoft YaHei", 0.006, length)
        set_font_size(self.ui.search_box, "Microsoft YaHei", 0.006, length)
        set_font_size(self.ui.music_name, "Microsoft YaHei", 0.006, length)
        set_font_size(self.ui.music_time_start, "Microsoft YaHei", 0.005, length)
        set_font_size(self.ui.music_time_end, "Microsoft YaHei", 0.005, length)
